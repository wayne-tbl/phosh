...
typedef struct {
  ...
  struct zphoc_blur_layer_surface_v1 *blur_surface;
...
set_blur (PhoshLayerSurface *self, guint radius)
{
  ...
  zphoc_blur_layer_surface_v1_set_blur (priv->blur_surface, radius);
  wl_surface_commit (priv->wl_surface);
}
...
phosh_layer_surface_map (GtkWidget *widget)
{
  ...
  priv->stacked_surface =
    zphoc_layer_shell_effects_v1_get_stacked_layer_surface (layer_shell_effects,
                                                            priv->layer_surface);
  if (wl_proxy_get_version ((struct wl_proxy *) layer_shell_effects) >=
      ZPHOC_LAYER_SHELL_EFFECTS_V1_GET_BLUR_LAYER_SURFACE_SINCE_VERSION) {
    priv->blur_surface =
      zphoc_layer_shell_effects_v1_get_blur_layer_surface (layer_shell_effects,
                                                           priv->layer_surface);
  }
...
phosh_layer_surface_unmap (GtkWidget *widget)
{
  ...
  g_clear_pointer (&priv->blur_surface, zphoc_blur_layer_surface_v1_destroy);
