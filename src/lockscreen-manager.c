...
typedef struct {
  char *folder;
  char *current;   /* basename of the wallpaper in use, may be NULL */
} RotateData;
...
/*
 * Pick the wallpaper after the one in use.
 *
 * Runs on a worker thread. Reading a directory is not free -- it is a syscall
 * per entry against whatever the folder happens to live on -- and this is
 * reached when the screen wakes, which is the moment the shell can least
 * afford to stall.
 *
 * Deliberately still a scan rather than a cache kept warm by a directory
 * monitor: it runs when the lock screen appears, not on a timer, and a cache
 * would have to survive the folder changing while the phone was asleep, being
 * on removable storage, or being swapped by the setting underneath it. Off the
 * main loop the scan costs nothing anyone can see.
 */
static void
 ...
 *
 * The folder is read on a worker thread, see rotate_wallpaper_scan().
 */
static void
